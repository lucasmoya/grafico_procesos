...
try:
    ...
    df["Avance_Esperado"] = (
        ((hoy - df[col_fecha_inicio]).dt.days / (df[col_tiempo_meses] * 30.44)) * 100
    ).clip(0, 100)
    ...
    gantt = alt.Chart(df).mark_bar(size=20).encode(
        ...
        tooltip=[
            ...
            alt.Tooltip("Fecha_Fin_Estimada", title="Fin Estimado"),
    ...
    st.altair_chart((gantt + linea_hoy).properties(height=300).interactive(), use_container_width=True)

    # ---------------- AVANCE REAL ----------------
    st.divider()
    st.markdown("### Avance Real por Proceso")
    ...
    st.data_editor(
        ...
        column_config={
            ...
            col_avance: st.column_config.ProgressColumn("Avance Real (%)", 0, 100),
            "Avance_Esperado": st.column_config.ProgressColumn("Avance Planificado (%)", 0, 100),
